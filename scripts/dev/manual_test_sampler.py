...
def show(title, spec):
    print(f"\n=== {title} ===")
    print(f"seed={spec.seed} given={spec.seed_was_given} wall={spec.wall_size} "
          f"n_holes={len(spec.holes)}")
    for i, h in enumerate(spec.holes):
        tag = "TARGET" if h.is_target else "distr "
        print(f"  [{tag}] shape={h.shape} pos=({h.pos[0]:+.3f},{h.pos[1]:+.3f}) "
...
show("explicit target + 3 distractors",
     sample_wall_spec(seed=1, true_hole={"pos": (0.0, 0.0), "size": {"diameter": 0.010},
                                         "chamfer": 0.002}, distractors=3))

# 4) Explicit list of distractor dicts (mixed: some posed, some sampled).
show("explicit distractor list",
     sample_wall_spec(seed=2,
