...
def main() -> int:
    ...
    for wp in [
        home_pos + d
        for d in [[0, 0.05, 0.05], [0, 0.05, -0.05], [0, -0.05, -0.05], [0, -0.05, 0.05]]
    ]:
    ...
    # Phase 3: force-trip
    ctrl.stiffness_tcp = np.array([2000.0, 2000.0, 2000.0, 3.0, 3.0, 3.0])
    ctrl.damping_tcp = np.array([180.0, 180.0, 180.0, 4.0, 4.0, 4.0])
    ...
    for _ in range(1500):  # 3 s
        ...
        if ctrl.status.state.value != "active":
            ...
            break
    # Reset gains.
    ctrl.stiffness_tcp = np.array([400.0, 400.0, 500.0, 3.0, 3.0, 3.0])
    ctrl.damping_tcp = np.array([80.0, 80.0, 89.0, 4.0, 4.0, 4.0])
    ...
    ax = np.zeros(3)
    mujoco.mju_subQuat(ax, home_quat, obs.ee_pose[3:])
    ...
    for step in range(4500):  # 9 s
        ...
        if step % 250 == 249:
            ...
            print(
                f"  t={obs.sim_time:6.2f}s  pos={pos_err:6.2f} mm  "
