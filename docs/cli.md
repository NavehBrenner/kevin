...
| `--control-mode {mirror,expo,rate}` | `expo` | Vision hand→arm mapping (`--input vision`): `expo` = position control with a dead-zone + soft centre (precise near rest, fast on big sweeps); `mirror` = plain linear position; `rate` = "point to steer" — hold a two-finger gesture (index+middle) and the arm flies in the direction you point, with a gentle forward creep as you angle the fingers into the camera; a fist drives slowly backward; an open/relaxed hand locks. Position-independent, low fatigue. |
