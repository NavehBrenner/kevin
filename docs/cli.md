...
| `--input MODE_OR_PATH` | `scripted` | Base command source: `scripted` noisy human, `vision` **two-webcam stereo** hand tracking (metric 3D + 6-DoF via the [stereohand](https://github.com/NavehBrenner/stereohand) package; needs the viewer, the `stereo-input` extra, and `--stereo-calib`), or a **path to a generated episode** (folder / `episode.npz`) to **reproduce** it: the scene and operator are rebuilt from the episode's seeds, so it reruns faithfully under any `--policy` (e.g. `--policy noassist` reproduces the human-only baseline, `--policy expert` the recorded run). |
