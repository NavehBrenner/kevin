...
Built in [MuJoCo](https://mujoco.org) for the Franka Emika Panda. The residual policy is trained via behavioral cloning against a scripted privileged-info expert. Three assistance modes — human-only, classical heuristic, learned residual — are compared head-to-head in a KPI ablation.
